...
const CORE_CACHE_VERSION = 'v39'; // Verified per-navigation ImagineDeck generation
    ...
    caches.open(CORE_CACHE_NAME).then((cache) => {
      ...
      return Promise.all(cachePromises).then(() => {
        if (failedInstallAssets.length > 0) {
          throw new Error('Core local assets failed to precache: ' + failedRequiredAssets.join(', '));
        }
      });
    })
  );
