...
const CORE_CACHE_VERSION = 'v38'; // Verified per-navigation ImagineDeck generation
...
self.addEventListener('install', (evt) => {
  ...
  evt.waitUntil(
    caches.open(CORE_CACHE_NAME).then((cache) => {
      ...
      const cachePromises = CORE_ASSETS_TO_CACHE.filter((assetUrl) => CORE_REQUIRED_ASSETS.has(assetUrl)).map((assetUrl) => {
        return (async () => {
          try {
      ...
      return Promise.all(cachePromises).then(() => {
        if (failedRequiredAssets.length > 0) {
          throw new Error('Required core assets failed to precache: ' + failedRequiredAssets.join(', '));
        }
      });
    })
  );
