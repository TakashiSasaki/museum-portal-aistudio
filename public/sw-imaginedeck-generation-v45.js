(() => {
  ...
  const IMAGINEDECK_GENERATION_VERSION = 45;
  ...
  async function ensureActiveGeneration() {
    ...
    return refreshStableGenerationFromNetwork();
  }
  ...
  async function currentVerifiedGeneration() {
    return readActiveGenerationPointer();
  }
  ...
  async function handleGenerationDocumentNavigation(request, evt) {
    ...
    try {
      ...
      generation = await currentVerifiedGeneration().catch(() => null);
    }
  ...
  handleNetworkFirstAssetRequest = function handleNetworkFirstAssetRequestV45(
    request,
    evt
  ) {
    const requestUrl = new URL(request.url);
